...
public class UnitController : MonoBehaviour {		
		...
		if(targetType == Target.None && anim.state != "Idle"){
			...
			if(movement.pathComplete){
				transform.LookAt(new Vector3(target.transform.position.x, transform.position.y, target.transform.position.z));
					...
					if(resource.Gather()){
						if(resource.behaviour[resource.source].returnWhenFull){
			...
			if(movement.pathComplete){
				...
				if(tState == TargetState.Undecided){
					...
					weapon.AttackObject(target, "Unit", type);
			...
			if(movement.pathComplete){
				...
				if(tState == TargetState.Self){
					...
					weapon.AttackObject(target, "Building", type);
	...
	public int DetermineRelations (int group){
		return mGroup.relations[group].state;
	}
