...
public class BuildingController : MonoBehaviour {
	...
	public float progressCur = 0;
	public float progressPerRate = 0;
	...
	public void FixedUpdate () {
		...
		if(buildingType == BuildingType.CompleteBuilding){
			if(unitProduction.canProduce){
				unitProduction.Produce(g, group);
			}
	...
	public void Damage (Type nType, int damage){
	...
	public void DisplayGUI (float ratioX, float ratioY) {
		...
		for(int x = 0; x < unitProduction.units.Length; x++){
			if(unitProduction.units[x].canProduce){
				if(bGUI.unitGUI.Display(x1, y1, unitProduction.units[x].customName, unitProduction.units[x].customTexture, ratioX, ratioY)){
		...
		for(int x = 0; x < techProduction.techs.Length; x++){
			if(techProduction.techs[x].canProduce && !g.tech[techProduction.techs[x].index].beingProduced){
				if(bGUI.technologyGUI.Display(x1, y1, techProduction.techs[x].customName, techProduction.techs[x].customTexture, ratioX, ratioY)){
		...
		for(int x = 0; x < unitProduction.jobsAmount; x++){
			if(bGUI.jobsGUI.Display(x1, y1, unitProduction.jobs[x].customName, unitProduction.jobs[x].customTexture, ratioX, ratioY)){
		...
		for(int x = 0; x < techProduction.jobsAmount; x++){
			if(bGUI.jobsGUI.Display(x1, y1, techProduction.jobs[x].customName, techProduction.jobs[x].customTexture, ratioX, ratioY)){
