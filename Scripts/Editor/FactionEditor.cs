/* The Faction Editor script extends the Unity Editor 
   and allows for simple creation and customization of
   factions, units, and buildings. You can find the
   editor under Window/Faction Editor. 
*/
...
public class FactionEditor : EditorWindow {
	...
	//State Controls for Displaying Options/GUI Fields
	int menuState = 0;
	int subMenuState = 0;
	int arraySelect = 0;
	int arraySelect1 = 0;
	...
	/*  Draws the Basic GUI for the Faction Editor
		This includes displaying factions available,
		changing groupState and */
	...
	void DrawSetup(Vector2 rect){
		...
		// Create a local array for the popup
							...
							// MiniMap
								...
								if(myTargetMap == null){
									...
									if(myTargetMap.enabled){
										myTargetMap.tag = EditorGUI.TextField(new Rect(400*rect.x, 100*rect.y, 540*rect.x, 25*rect.y), "Tag : ", myTargetMap.tag);
										...
										if(mapObj != null){
											...
											if(map != null){
												...
												for(int x = 0; x < map.elements.Length; x++){
													if(map.elements[x].tag == myTargetMap.tag){
						...
						// Technology
							...
							if(nTarget.tech.Length > 0 && myTarget.techEffect.Length > 0){
								...
								if(arraySelect >= myTarget.techEffect.Length){
									...
									curY += 25;
									string[,] functions = 	new string[23,4]{
												/* 0 */		{"GetName", "SetName", "", ""}, 
									...
									string[] variableName = {"Name", "Max Health", "Health", "Group", 
															 ...
															 "Speed", "Rotate Speed", "Mini Map Tag", "Vision Radius"
															};
			...
			if(unitId >= nTarget.buildingList.Length || unitId < 0){
				...
				if(nTarget.buildingList[unitId].obj){
					...
					if(myTarget){
						...
						objEditor.OnInteractivePreviewGUI(new Rect(945*rect.x, 50*rect.y, 555*rect.x, 650*rect.y), EditorStyles.toolbarButton);
							GUI.DrawTexture(new Rect(400*rect.x, 0, 275*rect.x, 25*rect.y), selectionTexture, ScaleMode.StretchToFill);
							if(GUI.Button(new Rect(400*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), "Global")){
								subMenuState = 0;
								arraySelect = 0;
							}
							if(GUI.Button(new Rect(675*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), "Production")){
								subMenuState = 2;
								arraySelect = 0;
							}
							if(GUI.Button(new Rect(950*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), "Size")){
								subMenuState = 3;
								arraySelect = 0;
							}
							if(GUI.Button(new Rect(1225*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), "Generate")){
								subMenuState = 4;
								arraySelect = 0;
							}
							// Global
							if(subMenuState == 0){		
								GUI.DrawTexture(new Rect(400*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), selectionTexture, ScaleMode.StretchToFill);
							...
							// Production
								...
								GUI.DrawTexture(new Rect(675*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), selectionTexture, ScaleMode.StretchToFill);
							...
							// Size
								...
								GUI.DrawTexture(new Rect(950*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), selectionTexture, ScaleMode.StretchToFill);
								nTarget.buildingList[unitId].closeSize = EditorGUI.IntField(new Rect(400*rect.x, 70*rect.y, 540*rect.x, 25*rect.y), "Close Size : ", nTarget.buildingList[unitId].closeSize);
							}
							// Generate
							else if(subMenuState == 4){
								GUI.DrawTexture(new Rect(1225*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), selectionTexture, ScaleMode.StretchToFill);
						...
						// GUI
							...
							objEditor.OnInteractivePreviewGUI(new Rect(945*rect.x, 50*rect.y, 555*rect.x, 650*rect.y), EditorStyles.toolbarButton);
							GUI.DrawTexture(new Rect(675*rect.x, 0, 275*rect.x, 25*rect.y), selectionTexture, ScaleMode.StretchToFill);
							if(GUI.Button(new Rect(400*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), "Selected")){
								subMenuState = 0;
								arraySelect = 0;
							}
							if(GUI.Button(new Rect(675*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), "Production GUI")){
								subMenuState = 1;
								arraySelect = 0;
							}
							if(GUI.Button(new Rect(950*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), "MiniMap")){
								subMenuState = 2;
								arraySelect = 0;
							}
							if(GUI.Button(new Rect(1225*rect.x, 25*rect.y, 275*rect.x, 20*rect.y), "Vision")){
								subMenuState = 3;
								arraySelect = 0;
							}
							// Selected
